...
class Breakpoint {
    ...
    get features() {
        if (!this._features) {
            this._features = Object.keys(this)
                .filter(key => key[0] !== '_')
                .sort();
        }
    ...
    set features(value = {}) {
        ...
        Object.keys(value).forEach(
            feature => (this[normalizeName(feature)] = value[feature]),
        );
    }
     ...
     */
    get smallest() {
        var smallest = this.features.reduce(
            (prev, f) => Math.min(prev, this.real(f)),
    ...
    measure(ctx = document.body) {
        ...
        m.style.cssText =
            'position:absolute;padding:0;margin:0;top:0;left:0;height:0;';
        ctx.appendChild(m);
...
function unique(breakpoints) {
    ...
    return breakpoints.filter(
        b => (!b || lookup[b.id] ? false : (lookup[b.id] = true)),
    );
}
